...
--
-- \[ g = \sum_i \sum_{\alpha \mid s_i(\alpha) = 0} \alpha \log \left( S_i(\alpha, t) \right) \]
--
-- and also whether \(f\) has an elemantary integral.
residueReduce ::
  ( Polynomial p e c,
    Eq (p e c),
    Num (p e c),
...
residueReduce derivation (Function e d) = do
  ...
  where
    ...
    toTerm prs specials (i, s)
      | degree s == 0 = Just (s, 1)
      | degree d == i = Just (s, mapCoefficients (\c -> scale c 1) d)
